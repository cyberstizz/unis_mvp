...
public interface AdViewRepository extends JpaRepository<AdView, UUID> {
    ...
    // For referral shares: Sum from referred_artist_id (10% share)
    @Query("SELECT SUM(a.revenueShare) FROM AdView a WHERE a.referredArtist.userId = :artistId AND DATE(a.viewedAt) = :date")
