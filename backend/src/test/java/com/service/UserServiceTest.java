...
class UserServiceTest {
    ...
    void testRegister() {
        ...
        when(userRepository.save(any(User.class))).thenAnswer(invocation -> {
            ...
            saved.setPasswordHash("hashedrawpass");
            return saved;
        });
    ...
    void testRegisterInvalidArtist() {
        // No stubs needed—the test throws before using them
    ...
    @Test
    void testGetProfile() {
        when(userRepository.findByIdWithJurisdiction(testUserId)).thenReturn(Optional.of(testUser));
    ...
    void testUpdatePassword() {
        ...
        when(userRepository.findById(testUserId)).thenReturn(Optional.of(testUser));
        when(passwordEncoder.matches(oldPass, testUser.getPasswordHash())).thenReturn(true);  // Stub for service call ("hashedpass")
        ...
        assertNotNull(updated);
        verify(passwordEncoder).matches(oldPass, "hashedpass");  // Verify the stubbed call (service calls with "hashedpass")
